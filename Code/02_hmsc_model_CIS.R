...
#CISCO model 
#note no seine catches 
#one observation with shock 
#fyke only 4 obs 
...
cis_count_dat<-cis_count_dat %>%
  ungroup()

#transform 
cis_count_dat$lake_area_km2<-cis_count_dat$lake_area_ac/247 #change from acre to km2
...
dat_shock<-filter(cis_count_dat, fish_count_SHOCK >0) %>%
  na.omit()
dat_fyke<-filter(cis_count_dat, fish_count_FT_NET >0) %>%
  na.omit() 
dat_gill<-filter(cis_count_dat, fish_count_GILL >0) %>% 
  na.omit() 
...
betaEsts[,1] <- fyke_betas$mean
betaEsts[,2] <- fyke_betas$`2.5%`
betaEsts[,3] <- fyke_betas$`97.5%` 
