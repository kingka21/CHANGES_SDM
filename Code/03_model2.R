...
dat<-read.csv("Data/contemp_lmb_dat.csv") 
...
#check out response: log of count is mostly normally dist, but negbinomial for dispersion 
...
pred_plot_log<-ggplot(data=plot_data, aes(x=log(col.means+1), y=log(fish_count_new + 1), xmax=log(upperCI.Group +1), xmin=log(lowerCI.Group +1) ) )+
  ...
  theme_bw()+theme(panel.grid = element_blank(), axis.title = element_text(size=20), axis.text = element_text(size=18)) + 
  geom_abline(intercept = 0, slope = 1, linetype = 'dashed')
