...
dat<-dplyr::select(dat, new_key, fish_count, logeffort, gear2, FMU_Code, z_lake_area, z_dd_mean, z_secchi, z_perim_km, z_order, 
                   z_ws_area_km2, z_ws_urban, z_ws_forest, z_ws_agriculture, z_ws_shrub, z_ws_wetland, z_ws_slope, z_ws_elevation, z_houses) %>%
  na.omit()
...
# Load data
data <- list(y = dat$fish_count, group = group$group.mean, gear=gear, n = dim(dat)[1], J = J, ngears = ngears,
             ...
             x12=dat$z_ws_slope, x13=dat$z_ws_elevation, x14=dat$z_houses,
...
#betas
bEst <- matrix(NA, nrow=14,ncol=3)
for(i in 1:14){ #parameters
