# modeling hierarchical model with varying intercepts only 
...
#plots 
plot(lmb_count_dat$lake_area_ac, lmb_count_dat$SHOCK) 
plot(lmb_count_dat$lake_perim_km, lmb_count_dat$SHOCK) 
plot(lmb_count_dat$lake_order, lmb_count_dat$SHOCK) 
plot(lmb_count_dat$ws_area_km2, lmb_count_dat$SHOCK)
plot(lmb_count_dat$ws_urban_prop, lmb_count_dat$SHOCK)
plot(lmb_count_dat$ws_agriculture_prop, lmb_count_dat$SHOCK)
plot(lmb_count_dat$ws_forest_prop, lmb_count_dat$SHOCK)
plot(lmb_count_dat$ws_shrub_prop, lmb_count_dat$SHOCK)
plot(lmb_count_dat$ws_wetland_prop, lmb_count_dat$SHOCK)
plot(lmb_count_dat$ws_slope_deg, lmb_count_dat$SHOCK)
plot(lmb_count_dat$ws_mean_elevation_m, lmb_count_dat$SHOCK)
plot(lmb_count_dat$dd_mean, lmb_count_dat$SHOCK)
plot(lmb_count_dat$houses_km, lmb_count_dat$SHOCK)
plot(lmb_count_dat$secchi_m, lmb_count_dat$SHOCK)

#transform 
lmb_count_dat$lake_area_km2<-lmb_count_dat$lake_area_ac/247 #change from acre to km2

#standardize and transform 
lmb_count_dat$z_order<-as.numeric(scale(log(lmb_count_dat$lake_order+0.001))) # min is 0 because these are the isolated
lmb_count_dat$z_lake_area<-as.numeric(scale(log(lmb_count_dat$lake_area_km2))) #good 
lmb_count_dat$z_perim_km<-as.numeric(scale(log(lmb_count_dat$lake_perim_km))) 
lmb_count_dat$z_ws_area_km2<-as.numeric(scale(log(lmb_count_dat$ws_area_km2)))  
lmb_count_dat$z_ws_urban<-as.numeric(scale(asin(sqrt(lmb_count_dat$ws_urban_prop)))) #good
lmb_count_dat$z_ws_agriculture<-as.numeric(scale(asin(sqrt(lmb_count_dat$ws_agriculture_prop)))) #ok
lmb_count_dat$z_ws_forest<-as.numeric(scale(asin(sqrt(lmb_count_dat$ws_forest_prop)))) #good
lmb_count_dat$z_ws_wetland<-as.numeric(scale(asin(sqrt(lmb_count_dat$ws_wetland_prop)))) #good
lmb_count_dat$z_ws_shrub<-as.numeric(scale(asin(sqrt(lmb_count_dat$ws_shrub_prop)))) #ok
lmb_count_dat$z_ws_slope<-as.numeric(scale(log(lmb_count_dat$ws_slope_deg + 0.001))) #has 0s so added 0.001 
lmb_count_dat$z_ws_elevation<-as.numeric(scale(log(lmb_count_dat$ws_mean_elevation_m))) #good 
lmb_count_dat$z_dd_mean<-as.numeric(scale(log(lmb_count_dat$dd_mean))) #good
lmb_count_dat$z_houses<-as.numeric(scale(log(lmb_count_dat$houses_km+ 0.001))) #ok 
...
dat_shock<-filter(lmb_count_dat, fish_count_SHOCK >0) %>% 
          dplyr::select(new_key, FMU_Code, fish_count_SHOCK, effort_SHOCK, z_lake_area, z_perim_km, z_order, z_ws_area_km2, z_ws_urban, z_ws_forest, z_ws_agriculture, z_ws_shrub, z_ws_wetland, z_ws_slope, z_ws_elevation, z_dd_mean, z_houses, z_secchi) %>%
          na.omit()
dat_fyke<-filter(lmb_count_dat, fish_count_FT_NET >0) %>% 
          dplyr::select(new_key, FMU_Code, fish_count_FT_NET, effort_FT_NET, z_lake_area, z_perim_km, z_order, z_ws_area_km2, z_ws_urban, z_ws_forest, z_ws_agriculture, z_ws_shrub, z_ws_wetland, z_ws_slope, z_ws_elevation, z_dd_mean, z_houses, z_secchi) %>% 
          na.omit() 
summary(dat_fyke)
dat_gill<-filter(lmb_count_dat, fish_count_GILL >0) %>% 
          dplyr::select(new_key, FMU_Code, fish_count_GILL, effort_GILL, z_lake_area, z_perim_km, z_order, z_ws_area_km2, z_ws_urban, z_ws_forest, z_ws_agriculture, z_ws_shrub, z_ws_wetland, z_ws_slope, z_ws_elevation, z_dd_mean, z_houses, z_secchi) %>% 
           na.omit() 
dat_seine<-filter(lmb_count_dat, fish_count_SEINE >0) %>% 
            dplyr::select(new_key, FMU_Code, fish_count_SEINE, effort_SEINE, z_lake_area, z_perim_km, z_order, z_ws_area_km2, z_ws_urban, z_ws_forest, z_ws_agriculture, z_ws_shrub, z_ws_wetland, z_ws_slope, z_ws_elevation, z_dd_mean, z_houses, z_secchi) %>% 
            na.omit() 
...
cat("
    ...
    log.lambda[i] <- 1 * offset[i] + alpha[group[i]] + b[1] * x1[i] + b[2] * x2[i] + b[3] * x3[i] + b[4] * x4[i] + 
    ...
    for(k in 1:14){
    b[k] ~ dnorm(0,prec.b[k])
    prec.b[k] <- 1/(s.b[k]*s.b[k]) 
    s.b[k] <- global.b*local.b[k]
    local.b[k] ~ dnorm(0,v.b[k])T(0,)
    v.b[k] ~ dgamma(0.5,0.5)
    }
    
    # Hyper-prior on gamma
...
J <- length(unique(dat_shock$FMU_Code))

# Create group index, must go from 1,..J
dat_shock$FMU_Code <- droplevels(as.factor(dat_shock$FMU_Code))
group <- as.numeric(as.factor(dat_shock$FMU_Code))

# Load data
data <- list(y = dat_shock$fish_count_SHOCK, group = group, n = dim(dat_shock)[1], J = J,
