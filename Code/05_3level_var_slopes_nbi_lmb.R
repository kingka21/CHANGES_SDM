...
#the betas to vary as a multivariate normal for each gear, 
#which means you would expect some correlations or dependencies of betas across environmental variables for the same gear
#load libraries 
library(R2jags)
library(lme4)
library(MCMCpack)
...
cat("
    ...
    # varying betas by gear of the model
    for(j in 1:ngears){
    b1[j] <- BB[j,1]
    b2[j] <-BB[j,2]
    b3[j] <-BB[j,3]
    ...
    BB.hat[j,1] <- mu.b1   # varying slope 
    BB.hat[j,2] <- mu.b2   # varying slope  
    BB.hat[j,3] <- mu.b3   # varying slope  
    BB.hat[j,4] <- mu.b4   # varying slope  
    BB.hat[j,5] <- mu.b5   # varying slope 
    BB.hat[j,6] <- mu.b6   # varying slope 
    BB.hat[j,7] <- mu.b7   # varying slope 
    BB.hat[j,8] <- mu.b8   #varying slope 
    BB.hat[j,9] <- mu.b9   # varying slope  
    BB.hat[j,10] <- mu.b10  # varying slope  
    }
    ...
    mu.b9 ~ dnorm(0, 0.0000001)
    mu.b10 ~ dnorm(0, 0.0000001)
 ...
 ### Model variance-covariance priors 
...
inits <- function (){
  list (mu.alpha = rnorm(8), mu.b1=rnorm(1), mu.b2=rnorm(1), mu.b3=rnorm(1), mu.b4=rnorm(1), 
  mu.b5=rnorm(1), mu.b6=rnorm(1), mu.b7=rnorm(1), mu.b8=rnorm(1), mu.b9=rnorm(1), mu.b10=rnorm(1), 
  BB=matrix(rnorm(ngears*K),nrow=ngears,ncol=K),Tau.B=rwish(K+1,diag(K)) )
}


# Parameters monitored
parameters <- c("alpha", "mu.alpha","tau.alpha", "mu.alpha2","tau.alpha2",'mu.b1', "mu.b2", "mu.b3", "mu.b4", "mu.b5", "mu.b6", "mu.b7", "mu.b8", "mu.b9", "mu.b10",
                'logq', 'r', "BB", "b1", "b2", "b3", "b4", "b5", "b6", "b7", "b8", "b9", "b10")


# MCMC settings 
ni <- 130000 # number of iterations 100000
nt <- 40 #number to thin 10
...
data <- list(y = dat$fish_count_new, group = group$group.mean, gear=gear, n = dim(dat)[1], J = J, ngears = ngears,
             ...
             logeffort=dat$logeffort, IND=dat$IND, nsites = nsites, site =site, K=K, W=W
) 
...
#save output 
saveRDS(output, "Data/output/output_lmb_nbinom_surface_varslopes.rds") 
...
for(i in 1:4){ #gears
  EstsSecchi[i,1] <- mean(output$BUGSoutput$sims.list$BB[,i,1])
  EstsSecchi[i,2:3] <- quantile(output$BUGSoutput$sims.list$BB[,i,1],c(0.05,0.95))
}
...
for(i in 1:4){ #gears
  Estsarea[i,1] <- mean(output$BUGSoutput$sims.list$BB[,i,2])
  Estsarea[i,2:3] <- quantile(output$BUGSoutput$sims.list$BB[,i,2],c(0.05,0.95))
}
...
for(i in 1:4){ #gears
  Eststemp[i,1] <- mean(output$BUGSoutput$sims.list$BB[,i,3])
  Eststemp[i,2:3] <- quantile(output$BUGSoutput$sims.list$BB[,i,3],c(0.05,0.95))
}
...
for(i in 1:4){ #gears
  Estsdepth[i,1] <- mean(output$BUGSoutput$sims.list$BB[,i,4])
  Estsdepth[i,2:3] <- quantile(output$BUGSoutput$sims.list$BB[,i,4],c(0.05,0.95))
}
...
for(i in 1:4){ #gears
  EstsDO[i,1] <- mean(output$BUGSoutput$sims.list$BB[,i,5])
  EstsDO[i,2:3] <- quantile(output$BUGSoutput$sims.list$BB[,i,5],c(0.05,0.95))
}
...
for(i in 1:4){ #gears
  EstsFor[i,1] <- mean(output$BUGSoutput$sims.list$BB[,i,6])
  EstsFor[i,2:3] <- quantile(output$BUGSoutput$sims.list$BB[,i,6],c(0.05,0.95))
}
...
for(i in 1:4){ #gears
  Estswet[i,1] <- mean(output$BUGSoutput$sims.list$BB[,i,7])
  Estswet[i,2:3] <- quantile(output$BUGSoutput$sims.list$BB[,i,7],c(0.05,0.95))
}
...
for(i in 1:4){ #gears
  Estsday[i,1] <- mean(output$BUGSoutput$sims.list$BB[,i,8])
  Estsday[i,2:3] <- quantile(output$BUGSoutput$sims.list$BB[,i,8],c(0.05,0.95))
}
...
for(i in 1:4){ #gears
  Estswae[i,1] <- mean(output$BUGSoutput$sims.list$BB[,i,9])
  Estswae[i,2:3] <- quantile(output$BUGSoutput$sims.list$BB[,i,9],c(0.05,0.95))
}
...
for(i in 1:4){ #gears
  Estspike[i,1] <- mean(output$BUGSoutput$sims.list$BB[,i,10])
  Estspike[i,2:3] <- quantile(output$BUGSoutput$sims.list$BB[,i,10],c(0.05,0.95))
}
...
#try mean of surface temps 
surface_temp<-read.csv("Data/MI_data/lake_surface_temp.csv") %>%
  dplyr:: select(IHDLKID, HECTARES, TAVE_1930:TAVE_1990) #keep the area, this will help with matching to lakes in DNR, some NHD lakes are split into multiple DNR lakes
surface_temp$surface_temp_mean<-rowMeans(surface_temp[,3:63])# take the means across the current years (1930-1990)
surface_mean<-dplyr::select(surface_temp, IHDLKID, HECTARES, surface_temp_mean) %>% 
  rename(nhdid=IHDLKID) %>% 
  group_by(nhdid) %>%
  slice_max(HECTARES) %>%#use the larger area for dups, usually separated basins
  ungroup() %>% left_join(lake_link)

hist_dat<- left_join(hist_dat, surface_mean)
#set up data 
...
hist_dat$z_surface_temp_mean<-as.numeric(scale(log(hist_dat$surface_temp_mean))) #good 
...
for(i in 1:nsim ){  #loop over sims 
  for(t in 1:length(hist_dat[[1]])){ #loop over covariate data (obs)
    predict_hist[i,t] <- exp(coefs[ID[i],paste0('mu.alpha[',hist_dat[['group']][t],']')] + coefs[ID[i],'b[1]']*hist_dat[['z_secchi']][t]  + coefs[ID[i],'b[2]']*hist_dat[['z_lake_area']][t]  + coefs[ID[i],'b[3]']*hist_dat[['z_surface_temp_mean']][t]  + 
