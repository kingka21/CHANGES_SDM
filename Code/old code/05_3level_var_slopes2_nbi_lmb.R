...
#correlations or dependencies of betas across gears for the same environmental variable
#load libraries 
library(R2jags)
library(lme4)
library(MCMCpack)
...
cat("
    ...
    for(k in 1:K){  # this is a loop to go across environmental variables
    BB[1:ngears,k] ~ dmnorm(BB.hat[,k], Tau.B[,]) # bivriate normal 
    
    BB.hat[1,k] <- mu.b1   # each gear 
    BB.hat[2,k] <- mu.b2    
    BB.hat[3,k] <- mu.b3    
    BB.hat[4,k] <- mu.b4     
    }
 ...
 ### Model variance-covariance priors 
     Tau.B[1:ngears,1:ngears] ~ dwish(W[,], df)
    df <- ngears+1
    Sigma.B[1:ngears,1:ngears] <- inverse(Tau.B[,])
    for (j in 1:ngears){
    for (j.prime in 1:ngears){
    rho.B[j,j.prime] <- Sigma.B[j,j.prime]/
    sqrt(Sigma.B[j,j]*Sigma.B[j.prime,j.prime])
    }
    sigma.B[j] <- sqrt(Sigma.B[j,j])
    }
...
inits <- function (){
  ...
  BB=matrix(rnorm(ngears*K),nrow=ngears,ncol=K),Tau.B=rwish(ngears+1,diag(ngears)) )
}


# Parameters monitored
parameters <- c("alpha", "mu.alpha","tau.alpha", "mu.alpha2","tau.alpha2",'mu.b1', "mu.b2", "mu.b3", "mu.b4", 
                'logq', 'r', "BB")


# MCMC settings 
ni <- 150000 # number of iterations 130000
nt <- 25 #number to thin 20
...
#save output 
saveRDS(output, "Data/output/output_lmb_nbinom_surface_varslopes2.rds") 
