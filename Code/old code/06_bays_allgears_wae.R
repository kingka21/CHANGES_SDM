...
#### data ####
dat <-wae_dat_for_model
#shoreline development factor; 
#calculated as lake_perimeter_m divided by the product of 2 times the square root of pi times lake_area_ha
dat$lake_perim_m <- dat$lake_perim_km * 1000
dat$lake_area_ha<-dat$lake_area_ac /2.471
dat$shore_complex<-dat$lake_perim_m / ( 2* (sqrt(3.1415 * dat$lake_area_ha)) ) 

## standardize and transform predictor values ##
dat$z_lake_area<-as.numeric(scale(log(dat$lake_area_m2))) #good 
dat$z_shore_complex<-as.numeric(scale(log(dat$shore_complex))) #good 
...
dat<-dplyr::select(dat, new_key, fish_count_new, logeffort, gear2, FMU_Code, 
                   z_lake_area, z_shore_complex, z_secchi, z_julian, 
                   z_surface_temp, z_bottom_do, 
                   z_ws_agriculture, z_ws_wetland) %>%
  na.omit()

n_distinct(dat$new_key) #255 lakes (getting rid of depth gives more lakes)
...
icc_n / icc_d #variance among regions 10%  
...
data <- list(y = dat$fish_count_new, group = dat$group, gear=gear, n = dim(dat)[1], J = J, ngears = ngears,
             x1=dat$z_secchi, x2=dat$z_lake_area, x3=dat$z_surface_temp, x4=dat$z_shore_complex, x5=dat$z_julian, x6=dat$z_bottom_do,
             x7=dat$z_ws_agriculture, x8=dat$z_ws_wetland,
...
saveRDS(output, "Data/output/model_output_wae.rds") 
