...
for(i in 1:4){ #gears
  Estspike[i,1] <- mean(output$BUGSoutput$sims.list$b9[,i])
  Estspike[i,2:3] <- quantile(output$BUGSoutput$sims.list$b9[,i],c(0.05,0.95))
}
...
all_est<- gtools::smartbind(EstsSecchi, Estsarea, Eststemp, Estsdepth, EstsDO, EstsFor, Estswet, Estsday, Estspike)
...
coefs <- output$BUGSoutput$sims.matrix[,1:267] 
...
for(i in 1:nsim ){  #loop over sims 
  for(t in 1:length(dat[[1]])){ #loop over covariate data (obs)
    predictions_sim[i,t] <- exp(coefs[ID[i],paste0('alpha[',data[['site']][t],']')] + coefs[ID[i], paste0('b1[',data[['gear']][t],']')]*data[['x1']][t]  + coefs[ID[i],paste0('b2[',data[['gear']][t],']')]*data[['x2']][t]  + coefs[ID[i],paste0('b3[',data[['gear']][t],']')]*data[['x3']][t]  + 
                                  ...
                                  coefs[ID[i],paste0('b8[',data[['gear']][t],']')]*data[['x8']][t] + coefs[ID[i],paste0('b9[',data[['gear']][t],']')]*data[['x9']][t] + 
...
obs_catch<-dplyr::select(dat, new_key, site, gear2, fish_count_new) %>% 
...
pred_plot_log<-ggplot()+
  geom_pointrange(data=plot_data, aes(x=log(col.means+1), y=log(fish_count_new + 1), xmax=log(upperCI.Group +1), xmin=log(lowerCI.Group +1) ) )+ 
  xlab('predicted catch log')+
  ylab('observed catch log')+
  theme_bw()+theme(panel.grid = element_blank(), axis.title = element_text(size=16), axis.text = element_text(size=14)) + 
  geom_abline(intercept = 0, slope = 1)

model2_pred_obs<-pred_plot_log+facet_wrap(~ gear2, ncol=2, scales = 'free') #allow scales to vary 
...
#ppc_hist(y, yrep[1:5, ])
#group 
model2_dens<-ppc_dens_overlay_grouped(y, yrep[1:50, ], group = dat$gear2) + xlim(0, 20)
...
for(i in 1:nsim ){  #loop over sims 
  for(t in 1:length(hist_dat[[1]])){ #loop over covariate data (obs)
    predict_hist[i,t] <- exp(coefs[ID[i],paste0('mu.alpha[',hist_dat[['group']][t],']')] + coefs[ID[i],paste0('b1[',hist_dat[['gear_index']][t],']')]*hist_dat[['z_secchi']][t]  + coefs[ID[i],paste0('b2[',hist_dat[['gear_index']][t],']')]*hist_dat[['z_lake_area']][t]  + coefs[ID[i],paste0('b3[',hist_dat[['gear_index']][t],']')]*hist_dat[['z_surface_temp_year']][t]  + 
                               ...
                               coefs[ID[i],paste0('b8[',hist_dat[['gear_index']][t],']')]*hist_dat[['z_julian']][t] + coefs[ID[i],paste0('b9[',hist_dat[['gear_index']][t],']')]*hist_dat[['pres_pike']][t] + 
...
hist_catch<-dplyr::select(hist_dat, new_key, site, gear, largemouthbass_sum) %>% 
  mutate(row = row_number())
...
round(median(bayesR2res), 2) #0.43
...
pike<-hist_dat %>% 
  select(new_key, pike_sum) %>% 
  group_by(new_key) %>% 
  summarise(across(where(is.numeric),sum))

hist_just_lake<-hist_dat %>% 
  distinct(new_key, .keep_all = TRUE) %>% #147 lakes 
  left_join(pike, by = "new_key") %>% 
  mutate(pres_pike = ifelse(pike_sum.y >= 1, 1, 0)
  )
...
for(i in 1:nsim ){  #loop over sims 
  for(t in 1:length(hist_just_lake[[1]])){ #loop over covariate data (obs)
    predict_hist[i,t] <- exp(coefs[ID[i],paste0('mu.alpha[',hist_just_lake[['group']][t],']')] + coefs[ID[i], paste0('b1[',hist_just_lake[['gear_index']][t],']')]*hist_just_lake[['z_secchi']][t]  + coefs[ID[i],paste0('b2[',hist_just_lake[['gear_index']][t],']')]*hist_just_lake[['z_lake_area']][t]  + coefs[ID[i],paste0('b3[',hist_just_lake[['gear_index']][t],']')]*hist_just_lake[['z_surface_temp_year']][t]  + 
                                  coefs[ID[i],paste0('b4[',hist_just_lake[['gear_index']][t],']')]*hist_just_lake[['z_max_depth']][t]  + coefs[ID[i],paste0('b5[',hist_just_lake[['gear_index']][t],']')]*hist_just_lake[['z_bottom_do']][t] + coefs[ID[i],paste0('b6[',hist_just_lake[['gear_index']][t],']')]*hist_just_lake[['z_ws_forest']][t] + coefs[ID[i],paste0('b7[',hist_just_lake[['gear_index']][t],']')]*hist_just_lake[['z_ws_wetland']][t] +
                                  coefs[ID[i],paste0('b8[',hist_just_lake[['gear_index']][t],']')]*hist_just_lake[['z_julian']][t] + coefs[ID[i],paste0('b9[',hist_just_lake[['gear_index']][t],']')]*hist_just_lake[['pres_pike']][t] )
  }
}
...
cont_just_lake<-dat %>% 
  ...
  left_join(pike_cont, by = "new_key") %>% 
  mutate(pres_pike = ifelse(pike_pres.y >= 1, 1, 0)
  ) %>% 
  mutate(
    gear_index = case_when( gear2 == "FT_NET" ~ 1,
                            gear2 == "GILL" ~ 2,
                            gear2 == "SEINE" ~ 3, 
                            gear2 == "SHOCK" ~ 4)
  )
...
for(i in 1:nsim ){  #loop over sims 
  for(t in 1:length(cont_just_lake[[1]])){ #loop over covariate data (obs)
    predict_cont[i,t] <- exp(coefs[ID[i],paste0('alpha[',cont_just_lake[['site']][t],']')] + + coefs[ID[i], paste0('b1[',cont_just_lake[['gear_index']][t],']')]*cont_just_lake[['z_secchi']][t]  + coefs[ID[i],paste0('b2[',cont_just_lake[['gear_index']][t],']')]*cont_just_lake[['z_lake_area']][t]  + coefs[ID[i],paste0('b3[',cont_just_lake[['gear_index']][t],']')]*cont_just_lake[['z_surface_temp_year']][t]  + 
                               coefs[ID[i],paste0('b4[',cont_just_lake[['gear_index']][t],']')]*cont_just_lake[['z_max_depth']][t]  + coefs[ID[i],paste0('b5[',cont_just_lake[['gear_index']][t],']')]*cont_just_lake[['z_bottom_do']][t] + coefs[ID[i],paste0('b6[',cont_just_lake[['gear_index']][t],']')]*cont_just_lake[['z_ws_forest']][t] + coefs[ID[i],paste0('b7[',cont_just_lake[['gear_index']][t],']')]*cont_just_lake[['z_ws_wetland']][t] +
                               coefs[ID[i],paste0('b8[',cont_just_lake[['gear_index']][t],']')]*cont_just_lake[['z_julian']][t] + coefs[ID[i],paste0('b9[',cont_just_lake[['gear_index']][t],']')]*cont_just_lake[['pres_pike']][t] )
  } 
}
